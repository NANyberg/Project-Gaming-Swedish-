...
output:
  html_document:
    ...
    toc: yes
    toc_depth: 2
subtitle: Projekt - Statistisk Databehandlinga
---
```{r include=FALSE}
library(tidyverse)
library(rvest)
...
GamesDF <- read_csv("../Project-Gaming-Swedish-/Data/GamesDF.csv")

# Fixar UserScore - Chr values till numeric
GamesDF <- GamesDF %>%
  mutate(UserScore = as.numeric(UserScore))

# Fixar ReleaseDate - Chr values till time
GamesDF <- GamesDF %>%
  mutate(ReleaseDate = parse_date_time(ReleaseDate, order = "mdy"))
...
En ska också ta det försiktigt med metoden för skrapning utifall det finns ett intresse att publicera arbetet offentligt. Det finns olika regleringar som gör det ett brott att ta del och utnyttja data som är tagen från en hemsida man själv inte äger på något sätt. 
